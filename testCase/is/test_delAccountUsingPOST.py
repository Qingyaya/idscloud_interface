...
class test_delAccountUsingPOST(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cursor.execute("SELECT sp_code FROM tb_sprole WHERE usr_id = '%s' " % self.params['uids'])
        self.params['spCode']=(cursor.fetchone())[0]
        db.close()

    def test_delAccountUsingPOST(self):
        u"""is_删除账号"""
